...
contract Auction {
    ...
    function register(address _account, uint8 _token) public onlyOwner{ 
        ...
        bidders[msg.sender].token = _token;
    ...
    function bid(uint8 _price) public {
        ...
        if(bidderAddr == address(0) || _price < (currentPrice + rule.minimumStep) || currentBidder.token < _price)
        revert();
    ...
    function getDeposit() public {
        ...
        if(currentPrice == bidders[currentWinner].deposit)
	    state = State.CLOSED;
